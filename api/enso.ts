...
export async function getRoute(toToken: string, amountIn: BigInt) {
  const response = await fetch(
    `XXXXXXchainId=1&fromAddress=0xd8da6bf26964af9d7eed9e03e53415d37aa96045&amountIn=${amountIn.toString()}&slippage=300&tokenIn=0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee&tokenOut=${toToken}`
  );
...
export async function getRouteBundle(toTokens: string[], amountIn: BigInt) {
  const query = `XXXXXX?chainId=1&fromAddress=0xd8da6bf26964af9d7eed9e03e53415d37aa96045`;
  const amountSplit = (
...
export async function getPools(project: string) {
  ...
  const data = `XXXXX?protocol=${project}`;
...
export async function getPoolApy(poolAddress: string) {
  const data = `XXXXX?tokenAddress=${poolAddress}`;
